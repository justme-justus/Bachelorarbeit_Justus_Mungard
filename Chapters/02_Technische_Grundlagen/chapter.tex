...
Heutzutage üblich ist die so genannte Classless Inter-Domain Routing-Notation (\gls{CIDR}), welche IP-Addresse und Netzmaske vereint. Wenn man die genannten Beispiele aus Adresse und Subnetzmaske zusammenführt, erhält man die IP-Adresse in \gls{CIDR}-Notation:\\
1.2.3.4/24. /24 entspricht dabei den gesetzten Network bits.
...
Ein Beispiel für einen autoritativen Nameserver ist Bind \cite{liu2006dns}. Innerhalb des Nameservers werden Domains anhand von \textit{\gls{Zone}n(-Dateien)} verwaltet.
\newpage
...
Die in dieser Arbeit verwendeten \gls{VPN}-Technologien sind Internet Protocol Security (\gls{IPsec}) und OpenVPN. \gls{IPsec} wird u.a. für die Standort-Vernetzung genutzt, auch \gls{Site-to-Site} genannt, und arbeitet auf \glslink{OSI-Schichtenmodell}{OSI-Schicht} 3. Ebenso dient es zur \textit{Einwahl} in (Firmen-)Netzwerke. \gls{IPsec} wird meist im Geschäftsumfeld genutzt und es exisitieren viele herstellerabhängige Implementationen.\\
...
Angesprochene \gls{MTU}-Probleme (s. Kap. \ref{mtumss}) können sich bei einer getunnelten Verbindung auf Grund des zusätzliches \textit{Overheads} der Enkapsulierung zeigen. Eine IP-Fragmen\-tierung sollte optimalerweise verhindert werden.
