...
Mit Hilfe der IP-Adresse und Netzmaske lässt sich ermitteln, ob sich ein anderer Host im gleichen \textit{Subnetz} befindet. Dafür berechnet das Betriebssystem die eigene Netzwerkadresse per logical-AND aus eigener IP und Netzmaske:
...
Da in diesem Fall das Netzwerk außerhalb des eigenen liegt, muss das Paket \textit{geroutet} werden.
...
Man nutzt optimalerweise Path MTU Discovery oder beeinflusst die Maximum Segment Size (MSS) für TCP bei Initiierung der Verbindung, um Fragmentierung (bestmöglich) zu umgehen.
