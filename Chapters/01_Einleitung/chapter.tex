...
Die NetUse AG ist ein \todo{bla: kurz sagen, was ihr macht.}.
...
zu verlagern oder zu erweitern. Die Kunden der NetUSE AG sehen in der Cloud eine Möglichkeit, niedrigere Kosten, eine höhere Kostentransparenz und eine höhere technische Flexibilität zu realisieren. \todo{skalierbarkeit, flexibilität. wiederhole den abstract.}
\\
Die NetUSE AG unterstützt ihre Kunden bei der Migration in die Cloud. Hier stehen zwei Public Cloud-Anbieter im Fokus: Amazon Web Services (AWS) und Microsoft Azure (Azure). Weiterhin unterstützt die Firma Kunden beim Aufbau einer \textit{Private Cloud}, d.h. \todo{kurz ausführen}. Für Private Clouds setzt die NetUSE AG auf Red Hat OpenShift.
\\
In klassischen on premise IT Diensten erstreckt sich der Aufbau und Betrieb eines Dienstes über mehrere Teams, Für den Aufbau eines einzelnen IT Dienstes sind bei der NetUSE AG folgende Teams beteiligt: Server-Einbau und -Installation (Team \glqq{Infrastruktur}\grqq{}), Netzwerkanbindung (Team \glqq{Netzwerk}\grqq{}), Pflege von Firewall-Regeln (Team \glqq{Security}\grqq{}).\\
...
In der Public Cloud gibt es auch keine klassische Netzwerk-Infrastruktur \todo{kurz mal klassisch umreißen}: Broadcast-fähige Layer-2-Domänen, welche ursprünglich notwendig waren, um lokale Netzwerkteilnehmer (ARP [IPv4] bzw. Neighbor Discovery [IPv6]) zu finden, werden nicht mehr benötigt. Die Cloud \textit{kennt} naturgemäß alle Teilnehmer und es ist nach Belieben möglich, Konnektivität zwischen diesen zu ermöglichen. Netzwerke in der Cloud sind Software-Defined (SDN)\cite{Seidel2015}. Das hat zur Folge, dass gewohnte Werkzeuge eines Netzwerk-Ingenieurs wegfallen, während gleichzeitig neue dazu gekommen sind.

\section{Ziel}

Es existieren bereits proprietäre Lösungen am Markt, um eine Vernetzung über Cloud-Anbieter hinweg zu ermöglichen.\todo{hybrid cloud?} Sie fallen in die Kategorie \textit{Software-Defined Wide Area Network (SD-WAN)} und sind nicht interoperabel zu SD-WAN-Lösungen anderer Hersteller.\\
