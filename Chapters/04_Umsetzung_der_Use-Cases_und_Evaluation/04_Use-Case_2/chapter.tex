...
Somit kam die Frage auf, ob man das Deployment beschleunigen könnte. Da in Use-Case 2 viele Tests und Änderungen gemacht wurden, musste das Deployment oftmals komplett neu erfolgen, was Deployment-Zeiten von bis zu anderthalb Stunden (siehe \ref{xml-tunnel-parameters}) nach sich ziehen kann.\\
...
Unter der Annahme, dass Use-Case 1 bereits bereitgestellt wurde, ist die Laufzeit des Deployments effektiv:\\
...
Nach dem Deployment von Use-Case 2 wird geprüft, ob sich die Domain vpn.ba.mungard.de auflösen lässt. Dies passiert in Listing \ref{dns-resolution-client-vpn} in Azure Dublin.
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{bash}
