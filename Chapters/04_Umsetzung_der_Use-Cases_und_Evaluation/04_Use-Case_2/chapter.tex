...
Das Subnetz, in dem die OpenVPN-\gls{Client}s terminiert werden, wird wieder dynamisch aus dem IPAM alloziert.
%TC:ignore
\begin{listing}[h]
...
\end{minted}
\caption{IP-Adresse in CIDR-Notation des \gls{Roadwarrior}-\gls{Client}s}
...
\end{minted}
\caption{Kein Zugriff des \gls{Roadwarrior}-\gls{Client}s auf interne Ressourcen ohne VPN-Verbindung}
\label{internet-access-roadwarrior}
\end{listing}\FloatBarrier
%TC:endignore
Die MaxMind \gls{GeoIP}-Datenbank wurde auf dem DNS-Server installiert und in der Bind-Konfiguration referenziert (s. Listing \ref{bind-geoip-directory}).
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{bash}
...
\end{minted}
\caption{Konfiguration \gls{GeoIP} für Bind-Nameserver}
\label{bind-geoip-directory}
\end{listing}\FloatBarrier
%TC:endignore
Dadurch wurde ermöglicht, \gls{GeoIP}-Daten in einer Access Control List (ACL) zu nutzen und verschiedene Fälle zu definieren. Die ACL heißt in diesem Falle \glqq dublin\grqq{} und erlaubt Zugriffe aus der Stadt Dublin bzw. Zugriff für den TSIG-Key \glqq db-vpn.ba.mungard.de\grqq{} (s. Listing \ref{tsig-key-acl}). Dieser Key wird benötigt, um Zonen-Updates machen zu können und ist im entsprechenden Terraform-Modul hinterlegt.
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{text}
acl "dublin" {
...
\end{minted}
\caption{ACL \glqq dublin\grqq{} für \gls{GeoIP} und TSIG-Key}
\label{tsig-key-acl}
\end{listing}\FloatBarrier
%TC:endignore
Weiterhin existiert ein \textit{\gls{View}} \glqq Dublin\grqq{}, in dem die ACL und Zonen-Daten zusammengeführt werden (s. Listing \ref{view-dublin-bind}). Mit \textit{allow-update} werden Zonen-Updates explizit und ausschließlich für genannten TSIG-Key erlaubt.
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{text}
view "dublin" {
...
\end{minted}
\caption{\gls{View} \glqq dublin\grqq{} für Zusammenführung von ACL und Zonendatei}
...
\end{listing}\FloatBarrier
%TC:endignore
Weiterhin gibt es eine ACL \glqq match-all\grqq{}, welche für anfragende \gls{Client}s vorgesehen ist, die nicht per \gls{GeoIP} identifiziert werden. Durch das Netzwerk 0.0.0.0/0 treffen die ACL alle restlichen Teilnehmer des Internets (s. Listing \ref{view-matchall-bin}).
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{text}
acl "match-all" {
...
Im Terraform-Modul muss ein Provider \textit{pro} TSIG-Key konfiguriert werden. Der server-Parameter konfiguriert den Nameserver, der per DNS UPDATE editiert werden soll. Außerdem ist das Pre-Shared-Secret (TSIG-Key) hinterlegt. Es wird für die Domain vpn.ba.mungard.de die IP-Adresse (\textit{var.dublin\_ip}) des \gls{VPN-Konzentrator}s in Dublin (Azure) eingetragen, welche dynamisch zur Laufzeit entsteht (s. Listing \ref{tf-provider-dns}).
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{tf}
provider "dns" {
...
\end{minted}
\caption{\gls{View} \glqq dublin\grqq{} und \gls{View} \glqq match-all\grqq{}}
\label{bind-entries-after-tf}
\end{listing}\FloatBarrier
%TC:endignore

Listing \ref{zone-dump-dublin-internal} zeigt einen exemplarischen Zone-Dump für einen internen \gls{Client}, welcher über Dublin verbunden ist. Die interne Zone ist intern.ba.mungard.de. Alle Cloud-Standorte nutzen den gleichen Name-Server (ns1.intern.ba.mungard.de), welcher in der Private Cloud vorhanden ist.\\
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{bash}
...
Dies gilt für den Fall, falls das \gls{Deployment} noch nicht stattgefunden hat und der \gls{Client} nicht in der Nähe von Kiel ist, um per \gls{GeoIP}-ACL Zugriff auf die Auflösung von \textit{vpn.ba.mungard.de} erhält. Per \textit{pullfilter ignore redirect-gateway} wird verhindert, dass ein Default Gateway über den Tunnel signalisiert wird: Nur alle \textbf{bekannten} Routen kommen über die OpenVPN-Server-Konfiguration. Es findet kein \textit{full tunneling} statt.
%TC:ignore
\begin{listing}[h]
...
\end{minted}
\caption{Die OpenVPN-\gls{Client} Konfigurationsdatei (gekürzt)}
...
Die \gls{Roadwarrior} \gls{Client}s müssen in einem eigenen Subnetz \glqq hinter\grqq{} dem \gls{VPN-Konzentrator} terminiert werden. Dafür werden Routing-Einträge notwendig, damit AWS VPC bzw. Azure VNET die Information besitzen, über welchen \textit{Next-Hop} das \gls{Roadwarrior}-VPN-Subnetz erreicht werden kann. Durch \textit{Route Tables} stellt dies auch keine Hürde dar in Azure. AWS hingegen bietet keine Möglichkeit, Subnetze mit einer längeren Netzmaske in der Route Table zu hinterlegen, falls sie ein Subnetz des VPC CIDR darstellen würden. Man spricht auch von \glqq more specifics\grqq{}.\\
...
\newpage
\subsection{Untersuchung von \gls{GeoIP}-Daten}
...
\end{minted}
\caption{Python-Skript mit Modul maxminddb zur Untersuchung der \gls{GeoIP}-Datenbank}
...
Es wurde pro Cloud-Standort eine virtuelle Maschine hochgefahren, die einen \gls{Roadwarrior}-\gls{Client} simulieren soll. Diese \gls{Client}s haben \textbf{keinen} Zugriff auf interne Komponenten, sondern sind lediglich mit dem Internet verbunden, über welches VPN-Verbindungen zum jeweiligen \gls{VPN-Konzentrator} aufgebaut werden können.
...
\end{minted}
\caption{Ping von \gls{Roadwarrior}-\gls{Client} Azure $\rightarrow$ vpn.ba.mungard.de.}
...
\end{minted}
\caption{OpenVPN Verbindungs-Log des \gls{Roadwarrior}-\gls{Client}s}
