...
\end{minted}
\caption{IP-Adresse in \gls{CIDR}-Notation des \gls{Roadwarrior}-\gls{Client}s}
...
\end{minted}
\caption{Kein Zugriff des \gls{Roadwarrior}-\gls{Client}s auf interne Ressourcen ohne \gls{VPN}-Verbindung}
...
\end{minted}
\caption{\gls{ACL} \glqq dublin\grqq{} für \gls{GeoIP} und \gls{TSIG}-Key}
\label{tsig-key-acl}
\end{listing}\FloatBarrier
%TC:endignore
...
\end{minted}
\caption{\gls{View} \glqq dublin\grqq{} für Zusammenführung von \gls{ACL} und Zonendatei}
...
\end{minted}
\caption{\gls{ACL} \glqq dublin-internal\grqq{} für internes \gls{DNS}}
...
\end{minted}
\caption{\gls{ACL} \glqq match-all\grqq{} für alle übrigen Anfragen}
\label{view-matchall-bin}
\end{listing}\FloatBarrier
%TC:endignore

Im Terraform-Modul muss ein Provider \textit{pro} \gls{TSIG}-Key konfiguriert werden. Der server-Parameter konfiguriert den Nameserver, der per \gls{DNS} UPDATE editiert werden soll. Außerdem ist das Pre-Shared-Secret (\gls{TSIG}-Key) hinterlegt. Es wird für die Domain vpn.ba.mungard.de die IP-Adresse (\textit{var.dublin\_ip}) des \gls{VPN-Konzentrator}s in Dublin (Azure) eingetragen, welche dynamisch zur Laufzeit entsteht (s. Listing \ref{tf-provider-dns}).
%TC:ignore
\begin{listing}[h]
\begin{minted}[breaklines,frame=single]{tf}
provider "dns" {
...
\end{minted}
\caption{Terraform Modul zur Veränderung von \gls{DNS}-Zonen}
...
Die \gls{Roadwarrior} \gls{Client}s müssen in einem eigenen Subnetz \glqq hinter\grqq{} dem \gls{VPN-Konzentrator} terminiert werden. Dafür werden Routing-Einträge notwendig, damit AWS VPC bzw. Azure VNET die Information besitzen, über welchen \textit{Next-Hop} das \gls{Roadwarrior}-\gls{VPN}-Subnetz erreicht werden kann. Durch \textit{Route Tables} stellt dies auch keine Hürde dar in Azure. AWS hingegen bietet keine Möglichkeit, Subnetze mit einer längeren Netzmaske in der Route Table zu hinterlegen, falls sie ein Subnetz des VPC \gls{CIDR} darstellen würden. Man spricht auch von \glqq more specifics\grqq{}.\\
Im Beispiel von Abb. \ref{grafik:more_specific_not_allowed_aws} sei das VPC \gls{CIDR} 192.168.0.0/16 (Target: local). Es soll eine Route auf ein Netzwerk 192.168.100.0/24 über die AWS Instance \glqq i-06230ccaa[...]\grqq{} (Next-Hop) gesetzt werden, was eine more specific Route zum VPC \gls{CIDR} darstellt.
...
Wed May 12 11:27:27 2021 PUSH: Received control message: 'PUSH_REPLY,dhcp-option \gls{DNS} 192.168.201.1,route 10.32.0.0 255.255.0.0,route 10.33.0.0 255.255.0.0,route 192.168.201.0 255.255.255.0,dhcp-option DOMAIN intern.ba.mungard.de,route-gateway 10.32.1.1,topology subnet,ping 10,ping-restart 60,ifconfig 10.32.1.2 255.255.255.0'
...
May 12 11:27:27 update-systemd-resolved: Adding IPv4 \gls{DNS} Server 192.168.201.1
...
\end{minted}
\caption{\texttt{iptables}-Filterregeln für \gls{DNS}-Anfragen}
...
\end{minted}
\caption{Open\gls{VPN}-Verbindungsversucht mit \texttt{iptables}-Filterregel}
